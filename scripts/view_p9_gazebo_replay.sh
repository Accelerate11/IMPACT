...
run_dir="$(realpath -e -- "${1:?Usage: $0 P9_RUN_DIR}")"
record_dir="${run_dir}/gz_record"
[[ -f "${record_dir}/state.tlog" ]] || { echo "P9 Gazebo recording missing: ${record_dir}" >&2; exit 2; }
...
set -u
export GZ_PARTITION="xq_p9_replay_${USER:-wsl}_$(date -u +%Y%m%dT%H%M%SZ)_$$"
...
until [[ -n "${world_name}" ]]; do
  ...
  if grep -Fq '/world/xq_p9_integrity_gate/control' <<<"${services}"; then
    world_name=xq_p9_integrity_gate
  elif grep -Fq '/world/default/control' <<<"${services}"; then
    world_name=default
  fi
  kill -0 "${server_pid}" 2>/dev/null || { echo "Gazebo replay server exited." >&2; exit 3; }
  ((SECONDS < deadline)) || { echo "P9 replay world did not become ready." >&2; exit 4; }
...
sleep 4
kill -0 "${gui_pid}" 2>/dev/null || { echo "Gazebo GUI exited during startup." >&2; exit 5; }

camera_response="$(gz service -s /gui/move_to/pose --reqtype gz.msgs.GUICamera \
  --reptype gz.msgs.Boolean --timeout 5000 \
  --req 'pose: {position: {x: 0.0, y: 0.0, z: 16.0}, orientation: {x: 0.0, y: 0.70710678, z: 0.0, w: 0.70710678}} projection_type: "perspective"')"
grep -Fq 'data: true' <<<"${camera_response}" || { echo "Gazebo camera setup failed." >&2; exit 6; }
printf 'open_top=true\nwalls_preserved=true\ncamera_response=%s\n' "${camera_response}" \
  >"${run_dir}/gazebo-replay-view.txt"
...
echo "Gazebo P9 open-top wide/narrow certification world is running."
wait "${gui_pid}" || true
