...
def main() -> int:
    ...
    vertical_excursion = {
        key: float(value["metrics"].get("maximum_vertical_excursion_m", 0.0))
        for key, value in p11.items()
    }
    simultaneous_lateral_vertical_excursion = {
        key: float(
            value["metrics"].get(
                "maximum_simultaneous_lateral_vertical_excursion_m", 0.0
            )
        )
        for key, value in p11.items()
    }
    ...
    common_checks = {
        "thresholds_frozen_before_formal_comparison": thresholds.get(
            "frozen_before_formal_comparison"
        )
        is True,
        ...
        "both_localization_ate_bounded": all(
            ate[key] <= float(thresholds["maximum_ate_rms_m"])
            for key in VARIANTS
        ),
        "integrity_vertical_maneuver_executed": vertical_excursion[
            "integrity_constrained"
        ]
        >= float(thresholds.get("minimum_integrity_vertical_excursion_m", 0.0)),
        "information_only_stays_below_vertical_limit": vertical_excursion[
            "information_only"
        ]
        <= float(thresholds.get("maximum_information_only_vertical_excursion_m", math.inf)),
        "integrity_compositional_maneuver_executed": simultaneous_lateral_vertical_excursion[
            "integrity_constrained"
        ]
        >= float(
            thresholds.get(
                "minimum_integrity_simultaneous_lateral_vertical_excursion_m", 0.0
            )
        ),
        "information_only_stays_below_compositional_limit": simultaneous_lateral_vertical_excursion[
            "information_only"
        ]
        <= float(
            thresholds.get(
                "maximum_information_only_simultaneous_lateral_vertical_excursion_m",
                math.inf,
            )
        ),
    ...
    summary = {
        ...
        "maximum_simultaneous_lateral_vertical_excursion_m": (
            simultaneous_lateral_vertical_excursion
        ),
