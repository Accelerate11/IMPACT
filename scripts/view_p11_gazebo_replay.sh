...
[[ -f "${record_dir}/state.tlog" ]] || { echo "P11 Gazebo recording missing." >&2; exit 2; }

set +u
source /opt/ros/humble/setup.bash
source "${workspace_root}/xq_install/setup.bash"
set -u
export GZ_PARTITION="xq_p11_replay_${USER:-wsl}_$(date -u +%Y%m%dT%H%M%SZ)_$$"
...
until [[ -n "${world_name}" ]]; do
  ...
  if grep -Fq '/world/xq_p11_integrity_exploration/control' <<<"${services}"; then
    world_name=xq_p11_integrity_exploration
  elif grep -Fq '/world/default/control' <<<"${services}"; then
    world_name=default
  fi
  kill -0 "${server_pid}" 2>/dev/null || { echo "P11 Gazebo replay server exited." >&2; exit 3; }
  ((SECONDS < deadline)) || { echo "P11 replay world did not become ready." >&2; exit 4; }
...
sleep 4
kill -0 "${gui_pid}" 2>/dev/null || { echo "P11 Gazebo GUI exited during startup." >&2; exit 5; }
...
grep -Fq 'data: true' <<<"${camera_response}" || { echo "P11 Gazebo camera setup failed." >&2; exit 6; }
...
echo "Gazebo P11 replay: open top, walls preserved, integrity-constrained exploration."
wait "${gui_pid}" || true
