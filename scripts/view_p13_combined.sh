...
setsid bash "${script_dir}/view_p13_gazebo_replay.sh" "${run_dir}" & pids+=("$!")
setsid bash "${script_dir}/view_p13_rviz.sh" "${run_dir}" & pids+=("$!")
echo "Opening Gazebo + RViz from the accepted P13 high-latency result. Press Ctrl+C to close both."
while :; do
