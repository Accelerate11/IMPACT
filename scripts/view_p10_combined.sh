...
setsid bash "${script_dir}/view_p10_gazebo_replay.sh" "${run_dir}" & pids+=("$!")
setsid bash "${script_dir}/view_p10_rviz.sh" "${run_dir}" & pids+=("$!")
echo "Opening P10 Gazebo + RViz dual replay. Close either window or press Ctrl+C."
while :; do
