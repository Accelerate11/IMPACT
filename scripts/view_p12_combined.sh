...
setsid bash "${script_dir}/view_p12_gazebo_replay.sh" "${run_dir}" & pids+=("$!")
setsid bash "${script_dir}/view_p12_rviz.sh" "${run_dir}" & pids+=("$!")
echo "Opening Gazebo + RViz from the same accepted P12 result. Press Ctrl+C to close both."
while :; do
