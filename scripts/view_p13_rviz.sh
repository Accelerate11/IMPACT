...
bag="${run_dir}/high_200ms/rosbag"
[[ -f "${bag}/metadata.yaml" ]] || { echo "P13 high-latency rosbag missing: ${bag}" >&2; exit 2; }
[[ -f "${workspace_root}/config/p13_replay.rviz" ]] || { echo "P13 RViz config missing." >&2; exit 2; }
...
log_prefix="/tmp/xq_p13_rviz_${ROS_DOMAIN_ID}"
...
setsid ros2 run xq_autonomy xq_p13_replay_visualizer --ros-args -p use_sim_time:=true \
  >"${log_prefix}_latency.log" 2>&1 < /dev/null & pids+=("$!")
...
echo "P13 RViz replay: ${run_dir}/high_200ms"
echo "Cyan=static, red=dynamic, green=selected trajectory, orange=FAST-LIO."
echo "Red overlay shows p99, raw/safe AL, margin and applied speed cap."
/opt/ros/humble/bin/rviz2 -d "${workspace_root}/config/p13_replay.rviz" \
  --ros-args -p use_sim_time:=true
