...
class P11FlightControllerNode(Node):
    ...
    def _make_candidates(self, now_s: float) -> None:
        ...
        safe_right = direct.copy()
        safe_right[:, 1] -= self._float("lateral_offset_m") * lateral_window
        safe_left = direct.copy()
        safe_left[:, 1] += self._float("lateral_offset_m") * lateral_window
        batch_id = 20261100 + self._segment_index
        trajectory_base = 202611000 + 10 * self._segment_index
        direct_message = self._bspline(direct, trajectory_base + 1, duration)
        safe_right_message = self._bspline(
            safe_right, trajectory_base + 2, duration
        )
        safe_left_message = self._bspline(
            safe_left, trajectory_base + 3, duration
        )
        self._candidates = [direct_message, safe_right_message, safe_left_message]
        direct_length = float(np.linalg.norm(np.diff(direct, axis=0), axis=1).sum())
        safe_right_length = float(
            np.linalg.norm(np.diff(safe_right, axis=0), axis=1).sum()
        )
        safe_left_length = float(
            np.linalg.norm(np.diff(safe_left, axis=0), axis=1).sum()
        )
        ...
        metadata.frontier_ids = ["frontier_main", "frontier_main", "frontier_main"]
        metadata.information_gains = [
            self._float("direct_information_gain"),
            self._float("safe_information_gain"),
            self._float("safe_information_gain"),
        ]
        metadata.travel_times_s = [duration, duration, duration]
        metadata.energy_costs = [
            self._planned_energy_spent + direct_length,
            self._planned_energy_spent + safe_right_length,
            self._planned_energy_spent + safe_left_length,
        ]
        metadata.return_energy_costs = [
            self._float("return_energy_cost"),
            self._float("return_energy_cost"),
            self._float("return_energy_cost"),
        ]
        metadata.collision_probabilities = [
            self._float("collision_probability"),
            self._float("collision_probability"),
            self._float("collision_probability"),
        ]
