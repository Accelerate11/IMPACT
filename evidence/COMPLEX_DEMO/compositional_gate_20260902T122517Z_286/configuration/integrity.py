...
class TemporalInformationMemory:
    """Bounded information-filter memory for observed LiDAR constraints.

    The input matrices are already expressed in the fixed map frame by
    FAST-LIO.  Accumulating them therefore rewards genuinely complementary
    surface normals collected from different viewpoints, while a repeated
    single-plane corridor remains weak along its unobserved direction.
    """

    def __init__(self, horizon_s: float = 0.0, maximum_equivalent_frames: float = 20.0):
        if (
            not math.isfinite(horizon_s)
            or not math.isfinite(maximum_equivalent_frames)
            or horizon_s < 0.0
            or maximum_equivalent_frames < 1.0
        ):
            raise ValueError("invalid temporal information-memory parameter")
        self.horizon_s = float(horizon_s)
        self.maximum_equivalent_frames = float(maximum_equivalent_frames)
        self._information: np.ndarray | None = None
        self._last_stamp_s = -math.inf
        self._equivalent_frames = 0.0

    @property
    def equivalent_frames(self) -> float:
        return self._equivalent_frames

    def update(self, information_matrix: np.ndarray, stamp_s: float) -> np.ndarray:
        matrix = np.asarray(information_matrix, dtype=float).reshape(3, 3)
        matrix = 0.5 * (matrix + matrix.T)
        if not math.isfinite(stamp_s) or not np.isfinite(matrix).all():
            raise ValueError("information memory input must be finite")
        if float(np.linalg.eigvalsh(matrix)[0]) < -1.0e-9:
            raise ValueError("information memory input must be positive semidefinite")
        if (
            self.horizon_s <= 0.0
            or self._information is None
            or stamp_s <= self._last_stamp_s
        ):
            self._information = matrix.copy()
            self._equivalent_frames = 1.0
        else:
            retention = math.exp(
                -(float(stamp_s) - self._last_stamp_s) / self.horizon_s
            )
            self._information = retention * self._information + matrix
            self._equivalent_frames = retention * self._equivalent_frames + 1.0
            if self._equivalent_frames > self.maximum_equivalent_frames:
                scale = self.maximum_equivalent_frames / self._equivalent_frames
                self._information *= scale
                self._equivalent_frames = self.maximum_equivalent_frames
